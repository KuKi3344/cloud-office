<template>
	<div>
		<el-table v-loading="loading" :data="records" class="el-table" height="400" border
			...
			<el-table-column prop="id" label="部门编号" width="300" height="40">
			</el-table-column>
			<el-table-column prop="name" label="部门名称" width="500" height="40">
			</el-table-column>
			<el-table-column label="操作" width="400" height="40">
		...
		<el-button plain icon="el-icon-circle-plus-outline" style="border-radius:20px;" @click="addPos">添加部门
		</el-button></div>
		
		
		<el-dialog style="width: 1000px;height: 1000px;" title="部门信息编辑" :visible.sync="zdydialog" append-to-body>
			<el-form :model="updateList">
				<el-form-item label="部门编号" width="80px" >
					<el-input class="dialog_input" v-model="updateList.id" :disabled="true"></el-input>
				</el-form-item>
				<el-form-item label="部门名称" width="80px">
		...
		<el-dialog style="width: 1000px;height: 1000px" title="部门信息添加" :visible.sync="addview" append-to-body>
...
<script>
	export default{
		...
		methods:{
			...
			update:function(){
				this.putRequest('/api/system/cfg/dept/update',this.updateList).then(resp=>{
				this.cancel();
			...
			deletePos:function(id){
				this.$confirm('此操作将永久删除该条信息, 是否继续?', '提示', {
						...
						this.$axios.delete('/api/system/cfg/dept/delete/'+id).then(resp=>{
							 this.cancel();
						})
				        }).catch(() => {
			...
			add:function(){
				this.postRequest('/api/system/cfg/dept/add',this.addList).then(resp=>{
					this.addview = false;
					this.addList.id = '';
					this.addList.name = '';
					this.cancel();
